...
Bunnicula is intended to enable the common messaging scenario where services and applications publish messages to an AMQP broker on the local LAN for speed and reliability, and those messages are then transmitted across a WAN to an AMQP broker
that will distribute the messages to remote consumers.

Bunnicula is designed to listen to messages published to any number of queues on a single source broker and distribute those messages to any number of target servers/exchanges.
